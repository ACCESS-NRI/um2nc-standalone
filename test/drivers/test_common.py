...
def test_DelayedCubePath__init__(p):
    delayed_path = drivers_common.DelayedCubePath(p)
...
def test_DelayedCubePath__get_var_name(varname, expected_error):
    ...
    if expected_error:
        with pytest.raises(expected_exception=expected_error):
            drivers_common.DelayedCubePath._get_var_name(cube)
    else:
        field_name = drivers_common.DelayedCubePath._get_var_name(cube)
        assert field_name == varname


def test_DelayedCubePath_resolve_cube():
    delayed_path = drivers_common.DelayedCubePath("parentdir/filename")

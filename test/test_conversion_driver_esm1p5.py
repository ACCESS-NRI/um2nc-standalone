...
@pytest.mark.parametrize(
    "ff_path,ff_date,nc_write_dir,expected",
    [
        (
...
def test_get_nc_write_path_recognized_unit(ff_path, ff_date, nc_write_dir, expected):
    """Check that netCDF file naming produces expected file paths for various
    expected unit keys.
    """
    nc_write_path = esm1p5_convert.get_nc_write_path(ff_path, nc_write_dir, ff_date)
...
def test_get_nc_write_path_unrecognized_unit():
    ...
    with pytest.warns(RuntimeWarning):
        nc_write_path = esm1p5_convert.get_nc_write_path(Path(ff_name), nc_write_dir, (ff_year, ff_month, 1))
...
def test_find_matching_fields_files():
    ...
    found_fields_files = esm1p5_convert.find_matching_fields_files(dir_contents, fields_file_pattern)
...
def test_convert_fields_file_list_success(mock_process, input_output_list):
...
def test_convert_fields_file_list_fail_excepted(mock_process_with_exception):
    mock_process_with_exception(um2nc.UnsupportedTimeSeriesError, "timeseries error")
    fake_input_output_paths = [(Path("fake_file"), Path("fake_file.nc"))]

    _, failed = esm1p5_convert.convert_fields_file_list(fake_input_output_paths)
...
def test_convert_fields_file_list_fail_critical(mock_process_with_exception):
    ...
    with pytest.raises(Exception) as exc_info:
        esm1p5_convert.convert_fields_file_list([("fake_file", "fake_file.nc")])
...
def test_convert_esm1p5_output_dir_error():
    with pytest.raises(FileNotFoundError):
        esm1p5_convert.convert_esm1p5_output_dir("/test_convert_esm1p5_output_dir_error/fake/path/")
...
def test_filter_naming_collisions(input_output_pairs, expected_pairs):
    ...
    with pytest.warns(match="Multiple inputs have same output path"):
        filtered_paths = list(esm1p5_convert.filter_name_collisions(input_output_pairs))
...
def test_format_failures_quiet_mode():
    ...
    formatted_failure_reports = list(esm1p5_convert.format_failures(failed, True))
...
def test_format_failures_standard_mode():
    ...
    formatted_failure_report_list = list(esm1p5_convert.format_failures(failed_conversion, quiet=False))
...
def test_success_fail_overlap():
    ...
    successes = [
        (success_only_path, Path("success_only.nc")),
